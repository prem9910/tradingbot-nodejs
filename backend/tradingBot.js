...
const checkTrade = () => {
  ...
  if (!buyPrice && balance >= currentPrice * 0.98) {
    ...
    position = Math.floor(balance / currentPrice);  // Convert all balance to stock (integer value)
    balance = balance - (position * currentPrice);  // Update balance after buying
...
const startTrading = () => {
  tradingActive = true;
  // Simulate receiving stock price updates in real-time
  const simulateRealTimeUpdates = () => {
    if (tradingActive) {
      checkTrade(); // Call checkTrade whenever a new price is received
      setTimeout(simulateRealTimeUpdates, 1000); // Simulate every second
    }
  };
  simulateRealTimeUpdates(); // Start the simulation
};

const stopTrading = () => {
  tradingActive = false;
  // No need to clear interval since we're using a recursive function
};
